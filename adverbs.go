...
func fold2Join(sep S, x V) V {
	...
	case S:
		return x
	case AS:
		return NewV(S(strings.Join([]string(xv), string(sep))))
	case AV:
		assertCanonical(xv)
...
func fold2Decode(f V, x V) V {
	...
	case F:
		...
		return fold2Decode(NewV(I(fv)), x)
	case AB:
...
func scan2Encode(f V, x V) V {
	...
	case F:
		...
		return scan2Encode(NewV(I(fv)), x)
	case AB:
