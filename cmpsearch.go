...
func memberOfAB(x V, y AB) V {
	...
	if t && f {
		...
		case array:
			...
			return newBV(r)
		default:
			return newBV(B2I(true))
		}
	}
	if t {
		return equal(x, newBV(B2I(true)))
	}
	return equal(x, newBV(B2I(false)))
}
...
func findS(s S, y V) V {
	switch y := y.BV.(type) {
	case S:
		return newBV(I(strings.Index(string(s), string(y))))
	case AS:
		r := make(AI, y.Len())
		for i, ss := range y {
